 ...
 *
 * Vendored verbatim from zmkfirmware/zephyr@v4.1.0+zmk-fixes
 * subsys/settings/src/settings_zms.c with one local addition:
 * settings_backend_init() retries zms_mount() after a flash_area_erase()
 * if the first mount attempt fails. This makes the storage region
 * self-heal on first install or after corruption — eliminating the need
 * for a manual `flash-erase-storage` step.
 *
 * Upstream Zephyr added the same capability as CONFIG_SETTINGS_ZMS_FORCE_MOUNT
 * via PR zephyrproject-rtos/zephyr#101348. That landed after Zephyr 4.1 was
 * cut, so it is not in v4.1.0+zmk-fixes.
 *
 * TODO: When ZMK rebases onto a Zephyr containing
 * CONFIG_SETTINGS_ZMS_FORCE_MOUNT (likely Zephyr 4.2+), delete this entire
 * `settings/` directory, drop CONFIG_SETTINGS_CUSTOM from the variant
 * defconfig, and set CONFIG_SETTINGS_ZMS_FORCE_MOUNT=y plus
 * CONFIG_SETTINGS_ZMS=y instead.
 */

#include <errno.h>
#include <string.h>

/* SETTINGS_CUSTOM=y disables SETTINGS_ZMS=y, which in turn drops the
 * SETTINGS_ZMS_* Kconfig symbols. settings_zms.h still references them.
 * Provide the upstream defaults so the header compiles. Matches
 * subsys/settings/Kconfig: SECTOR_SIZE_MULT=1, MAX_COLLISIONS_BITS=4.
 */
#ifndef CONFIG_SETTINGS_ZMS_SECTOR_SIZE_MULT
#define CONFIG_SETTINGS_ZMS_SECTOR_SIZE_MULT 1
#endif
#ifndef CONFIG_SETTINGS_ZMS_MAX_COLLISIONS_BITS
...
int settings_backend_init(void)
{
	...
	if (rc) {
		/* Mount failed — sectors are likely non-blank and non-ZMS-formatted
		 * (first install over old firmware, partial write after power loss,
		 * etc.). Wipe the partition and retry once. If the second attempt
		 * still fails, the underlying flash or partition config is broken
		 * and we propagate.
		 */
		int erase_rc;

		LOG_WRN("ZMS mount failed (%d); erasing partition and retrying", rc);
		erase_rc = flash_area_erase(fa, 0, fa->fa_size);
		if (erase_rc) {
			LOG_ERR("Settings partition erase failed: %d", erase_rc);
			return erase_rc;
		}

		rc = settings_zms_backend_init(&default_settings_zms);
		if (rc) {
			LOG_ERR("ZMS mount failed after erase: %d", rc);
			return rc;
		}
	}
